...
class ControllerRevolutionRevblog extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('revolution/revblog', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('revolution/revblog', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL')
		);

		$data['add'] = $this->url->link('revolution/revblog/add', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('revolution/revblog/delete', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$data['blogs'][] = array(
				...
				'edit'        => $this->url->link('revolution/revblog/edit', 'user_token=' . $this->session->data['user_token'] . '&revblog_id=' . $result['revblog_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_title'] = $this->url->link('revolution/revblog', 'user_token=' . $this->session->data['user_token'] . '&sort=bd.title' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('revolution/revblog', 'user_token=' . $this->session->data['user_token'] . '&sort=b.sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('revolution/revblog', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['revblog_id'])) {
			$data['action'] = $this->url->link('revolution/revblog/add', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('revolution/revblog/edit', 'user_token=' . $this->session->data['user_token'] . '&revblog_id=' . $this->request->get['revblog_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('revolution/revblog', 'user_token=' . $this->session->data['user_token'] . $url, 'SSL');
		...
		if (!empty($blog_info)) {
			$data['revblog_id'] = $blog_info['revblog_id'];
		} else {
			$data['revblog_id'] = 0;
		}
		$filter_blogs_data = array(
			'sort'  => 'bd.title',
			'order' => 'ASC',
			'start' => 0,
			'limit' => 9999
		);
		$data['blog_related'] = array();
		$data['blog_all'] = $this->model_revolution_revblog->getBlogs($filter_blogs_data);
		if (isset($this->request->post['blog_related'])) {
			$data['blog_related'] = $this->request->post['blog_related'];
		} elseif (isset($this->request->get['revblog_id'])) {
			$data['blog_related'] = $this->model_revolution_revblog->getBlogRelated($this->request->get['revblog_id']);
		} else {
			$data['blog_related'] = array();
		}
