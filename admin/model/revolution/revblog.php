...
class ModelRevolutionRevblog extends Model {
	public function addBlog($data) {
		...
		if (isset($data['blog_related'])) {
			foreach ($data['blog_related'] as $related_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "revblog_related SET revblog_id = '" . (int)$revblog_id . "', related_id = '" . (int)$related_id . "'");
			}
		}
		...
		if (isset($data['keyword'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET language_id = 1, query = 'revblog_id=" . (int)$revblog_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
		}

		$this->cache->delete('revblog');
		$this->cache->delete('revblog_blog');
		$this->cache->delete('blogmod_cache');
	...
	public function addBlogCategory($data) {
		...
		if (isset($data['keyword'])) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET language_id = 1, query = 'revblog_category_id=" . (int)$category_id . "', keyword = '" . $this->db->escape($data['keyword']) . "'");
		}
	...
	public function editBlog($revblog_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "revblog_related WHERE revblog_id = '" . (int)$revblog_id . "'");
		if (isset($data['blog_related'])) {
			foreach ($data['blog_related'] as $related_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "revblog_related SET revblog_id = '" . (int)$revblog_id . "', related_id = '" . (int)$related_id . "'");
			}
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "seo_url WHERE query = 'revblog_id=" . (int)$revblog_id . "'");
	...
	public function editBlogCategory($category_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "seo_url WHERE query = 'revblog_category_id=" . (int)$category_id . "'");
	...
	public function getBlogRelated($revblog_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "revblog_related WHERE revblog_id = '" . (int)$revblog_id . "'");
		foreach ($query->rows as $result) {
			$related_blog_data[] = $result['related_id'];
		}
